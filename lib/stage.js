...
Stage.prototype.begin = function begin () {
	if (!this.begun) {
		this.begun = true;
		...
		// Execute the predefined stages
		if (typeof this.sputnik.preStages[this.name] != 'undefined') {
			for (var i in this.sputnik.preStages[this.name]) {
				this.sputnik.preStages[this.name][i]();
			}
		}
		
	} else {
		log.error('Tried to begin stage ' + this.name + ' twice');
	}
}
...
Stage.prototype.when = function when (when, fnc, order) {
	...
	if (this.finished) {
		...
		log.verbose('Executed function meant before a stage after it had already begun', {level: 1});
		fnc();
	} else {
