...
Sputnik.prototype.defineStage = function defineStage (name, fnc) {
	...
	if (typeof this.preStages[name] == 'undefined') this.preStages[name] = [];
	
	// Add this function
	this.preStages[name].push(fnc);
}
...
Sputnik.prototype.begin = function begin (name) {
	...
	// And begin the stage (which fires the 'before' functions and predefined functions)
	stage.begin();
...
Sputnik.prototype.wait = function wait (stageName) {
	...
	if (typeof fnc == 'undefined') {
		log.error('Tried to make stage ' + stageName + ' wait on undefined function');
		return false;
	}
	
	if (typeof this.stages[stageName] == 'undefined') {
		if (this.options.strict) {
			log.error('Tried to wait for stage ' + stageName.bold + ' which has not been started', {level: 1});
			return false;
		} else {
	...
	if (!stage.open) {
		log.error('Tried to wait for function ' + fnc.name + ' in already counted stage: ' + stageName, {level: 1});
	} else {
