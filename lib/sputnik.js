...
/**
 * Execute a function before starting a certain stage, serial
 * 
 * @author   Jelle De Loecker   <jelle@kipdola.be>
 * @since    0.0.4
 * @version  0.0.4
 ...
 * @param    {Integer}  order     The order in which to execute this function
 */
Sputnik.prototype.beforeSerial = function before (stageName, fnc, order) {
	this.getStage(stageName).when('before-serial', fnc, order);
}
...
/**
 * Show all the stages that aren't ready yet
 * 
 * @author   Jelle De Loecker   <jelle@kipdola.be>
 * @since    0.0.3
 * @version  0.0.3
 *
 */
Sputnik.prototype.getBusy = function getBusy () {
	
	var stage, stageName, result = [];
	
	for (stageName in this.stages) {
		stage = this.stages[stageName];
		
		if (!stage.finished) {
			result.push(stageName);
		}
	}
